//! Smoke-test REPL: spawns claude-code-acp and pipes stdin lines through the
//! AgentBackend trait. Prints each AgentUpdate as it arrives.
//!
//! Usage: `cargo run -p agentline-agent-claude-code --example claude_code_repl`
//!
//! Requires `npx`, `node`, and a logged-in `claude` CLI on the host.
...
async fn main() -> anyhow::Result<()> {
    ...
    eprintln!("⏳ spawning claude-code-acp via npx (first run may download)…");
    let backend = spawn(ClaudeCodeConfig::default()).await?;
