//! Smoke-test REPL: spawns `kimi acp` and pipes stdin lines through the
//! AgentBackend trait. Prints each AgentUpdate as it arrives.
//!
//! Usage: `cargo run -p agentline-agent-kimi --example kimi_repl`
//!
...
async fn main() -> anyhow::Result<()> {
    ...
    eprintln!("⏳ spawning `kimi acp`…");
    let backend = spawn(KimiConfig::default()).await?;
