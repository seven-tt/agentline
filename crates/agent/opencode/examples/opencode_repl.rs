//! Smoke-test REPL: spawns `opencode acp` and pipes stdin lines through the
//! AgentBackend trait. Prints each AgentUpdate as it arrives.
//!
//! Usage: `cargo run -p agentline-agent-opencode --example opencode_repl`
//!
//! Requires `opencode` on PATH and a completed `opencode auth login`.

use agentline_agent_opencode::{OpencodeConfig, spawn};
...
async fn main() -> anyhow::Result<()> {
    ...
    eprintln!("⏳ spawning `opencode acp`…");
    let backend = spawn(OpencodeConfig::default()).await?;
