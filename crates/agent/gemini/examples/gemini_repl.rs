//! Smoke-test REPL: spawns `gemini --acp` and pipes stdin lines through the
//! AgentBackend trait. Prints each AgentUpdate as it arrives.
//!
//! Usage: `cargo run -p agentline-agent-gemini --example gemini_repl`
//!
//! Requires `gemini` on PATH and a prior interactive login.

use agentline_agent_gemini::{GeminiConfig, spawn};
...
async fn main() -> anyhow::Result<()> {
    ...
    eprintln!("⏳ spawning `gemini --acp`…");
    let backend = spawn(GeminiConfig::default()).await?;
