//! Smoke-test REPL: spawns `qodercli --acp` and pipes stdin lines through the
//! AgentBackend trait. Prints each AgentUpdate as it arrives.
//!
//! Usage: `cargo run -p agentline-agent-qoder --example qoder_repl`
//!
//! Requires `qodercli` on PATH and a completed `qodercli login` (or
//! `QODER_PERSONAL_ACCESS_TOKEN` set in the env).

use agentline_agent_qoder::{QoderConfig, spawn};
...
async fn main() -> anyhow::Result<()> {
    ...
    eprintln!("⏳ spawning `qodercli --acp`…");
    let backend = spawn(QoderConfig::default()).await?;
