...
- **孤儿进程清理**：shutdown 时通过 session 级 reaping 杀掉整个进程树（macOS `setsid` + `proc_listpids`）
- **PID 文件追踪**：上次崩溃残留的进程在启动时自动清理
- **单实例锁**：防止重复 daemon 抢同一个 IM token

### 运维
- 后台服务模式（launchd 托管，开机自启，崩溃自动拉起）
- 内嵌 Web Dashboard（状态、日志、微信扫码登录）
- macOS 菜单栏 app 快速控制 daemon
- 结构化日志，可配置级别
...
进程生命周期在 OS 层面管理：`setsid` 创建隔离 session，`kill_session` 收割整个进程树（即使子进程通过 `setpgid` 逃逸也能追回），`kill_on_drop` 作为最后安全网。
